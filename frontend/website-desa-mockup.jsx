    ...
    <div>
      ...
      {data.berita.length > 0 && (
        <div style={{ padding: "2.5rem 0", borderTop: `0.5px solid ${theme.line}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            {data.berita.slice(0, 3).map((b) => <PhotoCard key={b.id} title={b.title} subtitle={b.excerpt} tag={b.tag} />)}
          </div>
        </div>
      )}
      ...
      {data.produk.length > 0 && (
        <div style={{ padding: "2.5rem 0", borderTop: `0.5px solid ${theme.line}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            {data.produk.slice(0, 3).map((p) => <PhotoCard key={p.id} title={p.name} subtitle={`${p.harga}${p.unit} · ${p.umkm}`} />)}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ background: theme.panel, border: `0.5px solid ${theme.line}`, borderRadius: 14, padding: "1.25rem" }}>
            ...
            {(profil.struktur || []).map((r, i) => (
              <div key={i} style={{ padding: "7px 0", borderTop: `0.5px solid ${theme.line}`, fontSize: 13.5, color: theme.textSoft }}>{r}</div>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ padding: "2.5rem 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {berita.map((b) => (
          <div key={b.id} style={{ background: theme.panel, border: `0.5px solid ${theme.line}`, borderRadius: 14, padding: "1.25rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {produk.map((p) => (
          <div key={p.id} style={{ background: theme.panel, border: `0.5px solid ${theme.line}`, borderRadius: 14, overflow: "hidden" }}>
            <div style={{ height: 110, background: theme.goldSoft }} />
            <div style={{ padding: "1rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                ...
                <button style={{ fontSize: 12, border: `0.5px solid ${theme.pine}`, color: theme.pine, background: "transparent", borderRadius: 8, padding: "5px 10px", cursor: "pointer" }}>Pesan</button>
              </div>
            </div>
          </div>
        ))}
...
function AdminSambutan({ sambutan, token, reload }) {
  ...
  return (
    <AdminCard>
      ...
      <Field label="URL foto (opsional)" placeholder="https://…" value={form.foto_url || ""} onChange={set("foto_url")} />
      <div style={fieldWrap}>
    ...
    <AdminCard>
      ...
      {rows.map((r, i) => (
        <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.4fr auto", gap: 8, marginBottom: 8, alignItems: "center" }}>
          <input style={inputStyle} placeholder="Jabatan" value={r.jabatan || ""} onChange={(e) => update(i, "jabatan", e.target.value)} />
          <input style={inputStyle} placeholder="Nama" value={r.nama || ""} onChange={(e) => update(i, "nama", e.target.value)} />
          <input style={inputStyle} placeholder="URL foto (opsional)" value={r.foto_url || ""} onChange={(e) => update(i, "foto_url", e.target.value)} />
...
function AdminBerita({ berita, token, reload }) {
  const [form, setForm] = useState({ tag: "", title: "", tanggal: "", excerpt: "" });
  ...
  const save = async (row) => {
    setBusyId(row.id); setError("");
    try { await apiRequest(`/berita/${row.id}`, { method: "PUT", token, body: row }); await reload(); }
    catch (err) { setError(err.message); }
    setBusyId(null);
  };
  const remove = async (id) => {
  ...
  const add = async () => {
    ...
    setError("");
    try { await apiRequest("/berita", { method: "POST", token, body: form }); setForm({ tag: "", title: "", tanggal: "", excerpt: "" }); await reload(); }
    catch (err) { setError(err.message); }
  };
        ...
        {rows.map((b) => (
          <div key={b.id} style={{ borderTop: `0.5px solid ${theme.line}`, padding: "10px 0" }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
              <input style={{ ...inputStyle, fontWeight: 500 }} value={b.title} onChange={(e) => updateLocal(b.id, "title", e.target.value)} />
...
function AdminProduk({ produk, token, reload }) {
  const [form, setForm] = useState({ name: "", harga: "", unit: "", umkm: "" });
  ...
  const save = async (row) => {
    setBusyId(row.id); setError("");
    try { await apiRequest(`/produk/${row.id}`, { method: "PUT", token, body: row }); await reload(); }
    catch (err) { setError(err.message); }
    setBusyId(null);
  };
  const remove = async (id) => {
  ...
  const add = async () => {
    ...
    setError("");
    try { await apiRequest("/produk", { method: "POST", token, body: form }); setForm({ name: "", harga: "", unit: "", umkm: "" }); await reload(); }
    catch (err) { setError(err.message); }
  };
      ...
      <AdminCard>
        ...
        {rows.map((p) => (
          <div key={p.id} style={{ borderTop: `0.5px solid ${theme.line}`, padding: "10px 0" }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
              <input style={{ ...inputStyle, fontWeight: 500 }} value={p.name} onChange={(e) => updateLocal(p.id, "name", e.target.value)} />
  ...
  return (
    <>
      <AdminCard>
        ...
        <Field label="URL foto (opsional)" placeholder="https://…" value={form.fotoUrl} onChange={(e) => setForm({ ...form, fotoUrl: e.target.value })} />
        <div style={fieldWrap}>
  ...
  return (
    <>
      <AdminCard>
        ...
        <Field label="URL foto" placeholder="https://…" value={form.fotoUrl} onChange={(e) => setForm({ ...form, fotoUrl: e.target.value })} />
        <PrimaryBtn onClick={add}>Tambah foto</PrimaryBtn>
      </AdminCard>
      <AdminCard>
        <AdminRowHeader title={`Daftar foto (${rows.length})`} />
        {rows.map((g) => (
          <div key={g.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `0.5px solid ${theme.line}`, padding: "10px 0" }}>
    ...
    <div style={{ background: theme.bg, fontFamily: "'Inter', sans-serif", color: theme.text, minHeight: 400 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 2rem", borderBottom: `0.5px solid ${theme.line}`, position: "sticky", top: 0, background: theme.bg, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: 8, background: theme.pine, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 500, fontFamily: "'Sora', sans-serif" }}>SM</div>
